...
-- ============================================================
-- RLS POLICIES (fixed: INSERT uses WITH CHECK, not USING)
-- ============================================================
...
create policy "sessions_insert" on dump_sessions for insert with check (user_id = auth.uid());
create policy "sessions_update" on dump_sessions for update using (user_id = auth.uid());
create policy "sessions_delete" on dump_sessions for delete using (user_id = auth.uid());

-- thrones
create policy "thrones_read" on thrones for select using (auth.role() = 'authenticated');
create policy "thrones_insert" on thrones for insert with check (auth.role() = 'authenticated');
create policy "thrones_update" on thrones for update using (owner_user_id = auth.uid() or current_king_id = auth.uid());
create policy "thrones_delete" on thrones for delete using (owner_user_id = auth.uid());
...
create policy "friendships_update" on friendships for update using (user_id = auth.uid());
create policy "friendships_delete" on friendships for delete using (user_id = auth.uid());

-- user_achievements
create policy "achievements_read" on user_achievements for select using (auth.role() = 'authenticated');
create policy "achievements_insert" on user_achievements for insert with check (user_id = auth.uid());
create policy "achievements_update" on user_achievements for update using (user_id = auth.uid());

-- notification_events
create policy "notifications_read" on notification_events for select using (to_user_id = auth.uid() or from_user_id = auth.uid());
create policy "notifications_insert" on notification_events for insert with check (auth.role() = 'authenticated');
